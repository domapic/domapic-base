...
const { convertParametersToJSONSchema } = require('openapi-jsonschema-parameters')
...
const Api = function (core, middlewares, securityMethods) {
  ...
  let disabledSecurityMethod = null
  ...
  const ParamsValidator = function (methodDefinition) {
    const jsonSchemaParams = convertParametersToJSONSchema(toOpenApi2(_.clone(methodDefinition.parameters) || []))
  ...
  const ActionExecutor = function (operationId, responseCustomizator) {
    return function (req) {
      ...
      return Promise.resolve(operations[operationId].handler({
        path: req.params,
        query: req.query
      }, req.body, responseCustomizator, req.user))
    }
  }
  ...
  const addRouteMethod = function (methodDefinition, method, routePath) {
    ...
    if (methodDefinition.security && !_.isEmpty(methodDefinition.security)) {
      ...
      router.route(routePath)[methodToUse](new security.Anonymous())
    }
  ...
  const addRoute = function (routeDefinition, routePath) {
    return Promise.map(_.keys(routeDefinition), (method) => {
      ...
      // Add options method
      const parsedRouteDefinition = JSON.parse(JSON.stringify(routeDefinition).replace(/#/g, '/api/openapi.json#'))
  ...
  const addOpenApiRoute = function (openApiDefinition) {
    router.route('/openapi.json').get((req, res) => {
      ...
      middlewares.sendResponse(req, res, {
        ...openApiDefinition,
        servers: openApi.getServers(req)
      })
    })
  ...
  const initSecurity = function () {
    security = new Security(core, supportedSecurityMethods, authorizationRoles, disabledSecurityMethod)
    return Promise.resolve()
  }
    ...
    return ensureRouterNotInitialized()
      .then(() => {
        return Promise.map(_.keys(authentications), method => {
          if (method === 'disabled') {
            disabledSecurityMethod = authentications[method].verify
            return Promise.resolve()
          }
  ...
  const addCustomMiddlewares = () => {
    _.each(customMiddlewares, middleware => {
