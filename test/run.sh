...
function getOption {
  local  __resultvar=$1
  local  option=$2
  if [ "$3" = "$option" ] || [ "$4" = "$option" ] || [ "$5" = "$option" ] || [ "$6" = "$option" ] || [ "$7" = "$option" ] || [ "$8" = "$option" ] || [ "$9" = "$option" ] || [ "$10" = "$option" ]|| [ "$11" = "$option" ]; then
...
function setConfig {
  if [ ! $unique_test_to_launch = false ] && [ $integration = false ] && [ $end_to_end = false ]; then
    integration=true
    end_to_end=true
  fi

  if [ $integration = false ] && [ $end_to_end = false ] && [ $unit = false ] && [ $standard = false ]; then
    integration=true
    end_to_end=true
    unit=true
    standard=true
  ...
  if [ $unique_test_to_launch = false ] || [ "$unique_test_to_launch" = "$test_to_run" ]; then
    ...
    if [ $run_local = false ]; then
      ...
      if [ $run_service = true ]; then
        ...
        cd ..
        ./node_modules/.bin/_mocha -- --recursive test/$test_type/$test_to_run
        cd test
      fi
    fi
  fi
...
getTestToLaunch "$8"
getTestToLaunch "$9"
getOption run_service "--service" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
getOption standard "--standard" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
getOption unit "--unit" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
getOption integration "--integration" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
getOption end_to_end "--end-to-end" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
getOption develop "--develop" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
getOption build "--build" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
getOption run_local "--local" "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9"
...
if [ $unit = true ]; then
  cd ..
  set -e
  ./node_modules/istanbul/lib/cli.js cover ./node_modules/.bin/_mocha -- --recursive test/unit
  cd test
fi
if [ $integration = true ] || [ $end_to_end = true ]; then
